...
for v in RING_ADDR RING_SIZE TOTAL_ADDR WRITE_ADDR OMIT_ADDR; do
  [[ "${!v}" =~ ^0x[0-9a-fA-F]+$ ]] || { echo "ERROR: could not resolve $v from RAMLOG-SYMBOLS.txt" >&2; exit 1; }
done

printf 'RAM log ring: %s size %s\n' "$RING_ADDR" "$RING_SIZE"
printf 'Counters: total=%s write=%s omitted-NFW=%s\n' "$TOTAL_ADDR" "$WRITE_ADDR" "$OMIT_ADDR"

openocd \
  -f interface/stlink.cfg \
  -c "transport select hla_swd" \
  -f target/stm32l4x.cfg \
  -c "adapter speed 50; init; halt 3000; dump_image X0551026-ramlog-ring.bin $RING_ADDR $RING_SIZE; dump_image X0551026-ramlog-total.bin $TOTAL_ADDR 4; dump_image X0551026-ramlog-write.bin $WRITE_ADDR 4; dump_image X0551026-ramlog-nfw-omitted.bin $OMIT_ADDR 4; dump_image X0551026-sram-full.bin 0x20000000 0xA000; resume; shutdown"
